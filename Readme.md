...
- Application 的模式/业务代码只调用 Service；`app_system.c` 作为组合根负责编排初始化顺序。
...
- Bring-up 短路径使用 `APP_ENABLE_BRINGUP` 隔离，正式 Release 关闭。
...
- [ ] 完成基础 Bring-up：LED 闪烁、USART echo、IWDG 基础验证。
