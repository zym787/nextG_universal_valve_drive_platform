...
4. [Application 开发指南](application_development_guide.md)：模式编排、主循环入口、应用边界。
5. [Service 开发指南](service_development_guide.md)：协议、参数、运动、安全服务。
6. [Module 开发指南](module_development_guide.md)：EEPROM、电机、传感器、通信口适配。
7. [BSP 开发指南](bsp_development_guide.md)：MCU 外设封装、`bsp_status_t`、初始化入口。
8. [BoardConfig 开发指南](boardconfig_development_guide.md)：多板卡差异与 CubeMX 工程关系。
9. [ThirdParty 管理指南](thirdparty_management_guide.md)：第三方组件来源、命名、vendor/submodule 策略。
10. [软件版本与发布管理指南](software_version_release_guide.md)：Git tag、版本头文件、Release 产物。
11. [设计原则](concept.md)：实时性、内存、ISR、接口边界等项目原则。
...
- [ ] 实现 `mod_stepper_driver`，通过 BSP TIM/PWM/GPIO 操作 STEP/DIR/EN。
- [ ] 实现 `mod_sensor_input`，适配光感/限位/位置反馈。
- [ ] 实现 `mod_comm_port`，适配 USART/RS485，后续预留 CAN。
- [ ] 确认 Module 不出现协议解析、参数结构、阀门状态机和安全策略。

### Phase 4: Service

- [ ] 建立 `svc_status.h` 和必要 `svc_fault_t`。
- [ ] 实现 `svc_system_Init()` / `svc_system_Poll()`，聚合 BSP、Module、Service 初始化和轮询。
...
- [ ] 集成 ringbuffer，用于 USART 接收、协议输入和日志缓冲。
- [ ] 集成 event/message，初期用固定长度事件队列。
- [ ] 集成 Modbus RTU slave，先 vendor 固定版本，稳定后再评估 git submodule。
- [ ] 集成轻量 OSAL，只提供 tick、timeout、poll scheduler、event post/fetch。
- [ ] 集成轻量 log，Release 下可裁剪。
- [ ] 每个第三方组件记录 `LICENSE`、`SOURCE.txt`、版本或提交号。

### Phase 7: 测试与覆盖率

- [ ] 引入 Ceedling，统一管理 Unity、CMock、gcov/gcovr。
